...
import { getPopular, getTopRated, getAiringToday, getTrending, TMDBItem } from '../services/tmdb';
...
export default function TV() {
  ...
  const [allTV, setAllTV] = useState<TMDBItem[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [t, top, air, tr] = await Promise.all([
          getTrending('tv', 'day'),
          getTrending('tv', 'week'),
          getAiringToday(),
          getTopRated('tv'),
  ...
  useEffect(() => {
    const loadPage = async () => {
      setPageLoading(true);
      try {
        const res = await getPopular('tv', page);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-20">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pb-20">
        <h1 className="font-display text-4xl sm:text-5xl text-white mb-6 pt-4">TV Shows</h1>
        ...
        <Carousel title="Airing Today" items={airing} loading={loading} />
        <Carousel title="Top Rated" items={topRated} loading={loading} />

        <div className="py-6">
          <h2 className="section-title font-display text-2xl sm:text-3xl text-white mb-6 tracking-wide">Popular TV Shows</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {pageLoading
              ? Array.from({ length: 20 }).map((_, i) => (
                  <div key={i} className="skeleton rounded-md" style={{ aspectRatio: '2/3' }} />
                ))
              : allTV.map((item) => <MovieCard key={item.id} item={item} />)}
          </div>
